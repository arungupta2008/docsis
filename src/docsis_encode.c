...
int encode_uint(unsigned char *buf, void *tval, struct symbol_entry *sym_ptr) {
	...
	if (sym_ptr->low_limit || sym_ptr->high_limit) {
		if (helper->uintval < sym_ptr->low_limit
				|| helper->uintval > sym_ptr->high_limit) {
			fprintf(stderr,
					"docsis: at line %d, %s value %d out of range %hd-%hd\n ",
					line, sym_ptr->sym_ident, helper->uintval,
					sym_ptr->low_limit, sym_ptr->high_limit);
			exit(-15);
		}
	}
...
int encode_ip(unsigned char *buf, void *tval, struct symbol_entry *sym_ptr) {
	...
	if (!inet_aton(helper->strval, &in)) {
		fprintf(stderr, "Invalid IP address %s at line %d", helper->strval,
				line);
		exit(-1);
	}
#ifdef DEBUG
...
// Encode IP with Port number bith ipv6 and ipv4
int encode_ip_with_port(unsigned char *buf, void *tval,
		struct symbol_entry *sym_ptr) {
	if (buf == NULL) {
		fprintf(stderr, "encode_ip called w/NULL buffer\n");
		exit(-1);
	}
	...
	union t_val *helper; /* We only use this to cast the void* we receive to what we think it should be */
	helper = (union t_val *) tval;
	char *ip_with_port = helper->strval;
	char ip_with_port_[strlen(ip_with_port)];
	strcpy(ip_with_port_, ip_with_port);
	char *token;
	int count_of_colums = -1;
	int i = 0;
	int colonPos = 0;

	while (ip_with_port_[i]) {
		if (((char) ip_with_port_[i]) == ':')
			colonPos = i;
		++i;
	}

	char ip[60] = { '\0' };
	strncpy(ip, ip_with_port_, colonPos);
	token = strtok(ip_with_port_, ":");
	char *port;
	while (token) {
		port = token;
		token = strtok(NULL, ":");
	...
	token = NULL;
	// for ipv4 : will be only one in count_of_colums
	if (count_of_colums > 1) {
		u_int16_t port_int;
		char *end;
		port_int = strtol(port, &end, 10);
		struct in6_addr *addr;
		struct sockaddr_in6 sa;
		if (!inet_pton(AF_INET6, ip, &(sa.sin6_addr))) {
			fprintf(stderr, "Could not convert address");
			fprintf(stderr, "Invalid IP_with_Port address %s at line %d",
					helper->strval, line);
			exit(-1);
		}
		sa.sin6_port = port_int;

#ifdef DEBUG
		fprintf(stderr, "encode_ip_with_port: found %s at line %d\n",inet_ntoa(sa), line);
#endif /* DEBUG */
		memcpy(buf, &sa, sizeof(struct sockaddr_in6));
		free(helper->strval);
		return (sizeof(struct sockaddr_in6));

	} else if (count_of_colums == 1) {
		// Ipv4
		struct in_addr addr;
		// Remove Port
		u_int16_t port_int;
...
int encode_ip6_with_port(unsigned char *buf, void *tval,
		struct symbol_entry *sym_ptr) {
	struct in6_addr in;
	union t_val *helper; /* We only use this to cast the void* we receive to what we think it should be */
...
int encode_ether(unsigned char *buf, void *tval, struct symbol_entry *sym_ptr) {
	...
	if (!(retval = ether_aton(helper->strval, buf))) {
		fprintf(stderr, "Invalid MAC address %s at line %d", helper->strval,
				line);
		exit(-1);
	}
#ifdef DEBUG
...
int encode_dual_qtag(unsigned char *buf, void *tval,
		struct symbol_entry *sym_ptr) {
	int i, final;
	char *token;
	char *array[2];
...
int encode_dual_int(unsigned char *buf, void *tval,
		struct symbol_entry *sym_ptr) {
	short int i, final;
	char *token;
	char *array[2];
...
int encode_ethermask(unsigned char *buf, void *tval,
		struct symbol_entry *sym_ptr) {
	...
	if (mask == NULL) {
		fprintf(stderr,
				"encode_ethermask: at line %d, format should be <mac_address>/<mac_mask>\n",
				line);
		exit(-1);
	}
	mask[0] = 0x0;
	mask++; /* cut the string in two */
...
int encode_string(unsigned char *buf, void *tval, struct symbol_entry *sym_ptr) {
	...
	if (sym_ptr->low_limit || sym_ptr->high_limit) {
		if (string_size < sym_ptr->low_limit) {
			fprintf(stderr,
					"encode_string: String too short, must be min %d chars\n",
					sym_ptr->low_limit);
			exit(-1);
		}
		if (sym_ptr->high_limit < string_size) {
			fprintf(stderr,
					"encode_string: String too long (%d chars), must be max %d chars\n",
...
int encode_strzero(unsigned char *buf, void *tval, struct symbol_entry *sym_ptr) {
	...
	if (sym_ptr->low_limit || sym_ptr->high_limit) {
		...
		if (sym_ptr->high_limit < string_size) {
			fprintf(stderr,
					"encode_string: String too long, must be max %d chars\n",
					sym_ptr->high_limit);
			exit(-1);
		}
	}
...
int encode_hexstr(unsigned char *buf, void *tval, struct symbol_entry *sym_ptr) {
	...
	if (sym_ptr->low_limit || sym_ptr->high_limit) {
		if (i < sym_ptr->low_limit) {
			fprintf(stderr,
					"encode_hexstr: Hex value too short, must be min %d octets\n",
					sym_ptr->low_limit);
			exit(-1);
		}
		if (sym_ptr->high_limit < i) {
			fprintf(stderr,
					"encode_hexstr: Hex value too long, must be max %d octets\n",
					sym_ptr->high_limit);
			exit(-1);
		}
	}
...
int encode_ushort_list(unsigned char *buf, void *tval,
		struct symbol_entry *sym_ptr) {
	...
	do {
		...
		if (value_found > 65535) {
			fprintf(stderr,
					"Parse error at line %d: value cannot exceed 65535\n",
					line);
			exit(-11);
		}
		nr_found++;
